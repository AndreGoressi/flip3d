...
// Initialisation
// ============================================================================
bool Flip3DPrototypeApp::Initialize(HINSTANCE instance)
{
...
// Window procedure
// ============================================================================
LRESULT CALLBACK Flip3DPrototypeApp::WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
...
// Card model building
// ============================================================================
void Flip3DPrototypeApp::BuildCardModels()
{
    m_cards.clear();
...
bool Flip3DPrototypeApp::CreateAppWindow()
{
    ...
    DWORD exStyle = WS_EX_NOREDIRECTIONBITMAP | WS_EX_TOPMOST | WS_EX_TRANSPARENT | WS_EX_TOOLWINDOW;
...
HRESULT Flip3DPrototypeApp::CreateDeviceResources()
{
    HRESULT hr = S_OK;
...
void Flip3DPrototypeApp::OnGlobalTimeUpdated()
{
...
void Flip3DPrototypeApp::TickRepeatedRotate()
{
...
// ============================================================================
int Flip3DPrototypeApp::FrontCardIndex() const
{
    if (m_cards.empty()) return -1;
    return 0;
}

int Flip3DPrototypeApp::ResolveOriginalFrontIndex() const
{
...
int Flip3DPrototypeApp::DistanceBetween(size_t sourcePos, size_t targetPos, bool forward) const
{
...
void Flip3DPrototypeApp::RotateListPhysically(bool backward)
{
...
void Flip3DPrototypeApp::StartRotationStep(bool backward, float duration)
{
...
bool Flip3DPrototypeApp::IsInteractiveKeyboardState() const
{
    return m_state == ViewState::Enter || m_state == ViewState::Interactive
        || m_state == ViewState::InteractiveRepeatedRotate;
}

bool Flip3DPrototypeApp::IsSelectionKeyboardState() const
{
    return m_state == ViewState::Interactive || m_state == ViewState::InteractiveRepeatedRotate;
}

bool Flip3DPrototypeApp::IsSelectionInputState() const
{
    return IsSelectionKeyboardState();
}

float Flip3DPrototypeApp::RotationDurationForRotateList() const
{
...
bool Flip3DPrototypeApp::ShouldReverseHorizontalWheel() const
{
    return m_hwnd != nullptr && (GetWindowLongPtrW(m_hwnd, GWL_EXSTYLE) & WS_EX_LAYOUTRTL) != 0;
}

void Flip3DPrototypeApp::ReplayEnterAnimation()
{
    BuildCardModels();
    CreateWindowCaptures();
...
void Flip3DPrototypeApp::BeginExitView()
{
...
void Flip3DPrototypeApp::BeginExitAnimation() { BeginExitView(); }

// ============================================================================
// Selection
// ============================================================================
void Flip3DPrototypeApp::SelectWindow(HWND targetHwnd)
{
...
void Flip3DPrototypeApp::SelectFrontWindow()
{
...
// Rotation input
// ============================================================================
void Flip3DPrototypeApp::RotateListByMouseWheelAmount(int mouseWheelAmount)
{
...
void Flip3DPrototypeApp::RotateBy(int delta)
{
...
void Flip3DPrototypeApp::RotateTo(int /*targetIndex*/) { BeginRotateToWindow(nullptr); }

void Flip3DPrototypeApp::BeginRotateToWindow(HWND targetHwnd)
{
    const size_t count = m_cards.size();
...
// Hit testing
// ============================================================================
bool Flip3DPrototypeApp::IntersectRayTriangle(
...
int Flip3DPrototypeApp::HitTest3DScene(LONG x, LONG y) const
{
...
bool Flip3DPrototypeApp::SelectWindowAtPoint(LONG x, LONG y)
{
...
// ============================================================================
bool Flip3DPrototypeApp::ProcessMouseWheelInput(int mouseWheelAmount, bool horizontalWheel)
{
...
bool Flip3DPrototypeApp::ProcessMouseButtonInput(LONG x, LONG y, bool pressed)
{
...
bool Flip3DPrototypeApp::ProcessMouseInput(UINT message, WPARAM wParam, LPARAM lParam)
{
    switch (message)
    {
...
bool Flip3DPrototypeApp::ProcessKeyboardInput(bool isKeyDown, UINT vkCode, bool isRepeat)
{
    if (isKeyDown)
    {
...
// ============================================================================
float Flip3DPrototypeApp::EnterProgress() const
{
    return std::clamp(m_enterTimeline.Value(), 0.0f, 1.0f);
}

XMMATRIX Flip3DPrototypeApp::BuildViewMatrix(float enterProgress) const
{
...
XMMATRIX Flip3DPrototypeApp::BuildProjectionMatrix(float enterProgress) const
{
...
DrawBuildContext Flip3DPrototypeApp::CreateDrawBuildContext() const
{
...
float Flip3DPrototypeApp::SteadyOpacityForRelative(const DrawBuildContext &context, float relative) const
{
...
bool Flip3DPrototypeApp::IsVisibleInView(
...
std::vector<VisibleCardStructure> Flip3DPrototypeApp::BuildVisibleCardStructure(const DrawBuildContext &context) const
{
...
CardAnimationState Flip3DPrototypeApp::CreateBaseCardAnimationState(
...
CardAnimationState Flip3DPrototypeApp::ResolveOutgoingCardAnimationState(
...
CardAnimationState Flip3DPrototypeApp::ResolveIncomingCardAnimationState(
...
CardAnimationState Flip3DPrototypeApp::ResolveSteadyCardAnimationState(
    const VisibleCardStructure &entry, const DrawBuildContext &context,
    CardAnimationState state) const
{
...
CardAnimationState Flip3DPrototypeApp::ResolveCycleCardAnimationState(
    const DrawBuildContext &context, CardAnimationState state) const
{
    if (context.rotationSteps > 0)
    {
...
CardAnimationState Flip3DPrototypeApp::ResolveCardAnimationState(
...
CardWorldState Flip3DPrototypeApp::GetWorldFromParametric(
...
float Flip3DPrototypeApp::ResolveDrawItemWindowOpacity(
    const VisibleCardStructure &entry, const DrawBuildContext &context,
...
float Flip3DPrototypeApp::ApplyEnterProgressToWindowOpacity(float windowOpacity, float enterProgress) const
{
...
float Flip3DPrototypeApp::ResolveDrawItemStateOpacity(const CardModel &card, float enterProgress) const
{
...
float Flip3DPrototypeApp::ResolveOutgoingTransitionOpacity(const DrawBuildContext &context) const
{
...
float Flip3DPrototypeApp::ResolveIncomingTransitionOpacity(const DrawBuildContext &context) const
{
...
float Flip3DPrototypeApp::ResolveCycleTransitionOpacity(const DrawBuildContext &context) const
{
...
float Flip3DPrototypeApp::ResolveDrawItemTransitionOpacity(
    const VisibleCardStructure &entry, const DrawBuildContext &context) const
{
...
float Flip3DPrototypeApp::UpdateDrawItemAlpha(
    const VisibleCardStructure &entry, const DrawBuildContext &context,
...
bool Flip3DPrototypeApp::TryBuildDrawItemForStructure(
    const VisibleCardStructure &entry, const DrawBuildContext &context,
...
std::vector<DrawItem> Flip3DPrototypeApp::BuildDrawItems(float enterProgress) const
{
    const DrawBuildContext context = CreateDrawBuildContext();
...
// ============================================================================
LRESULT Flip3DPrototypeApp::HandleMessage(UINT message, WPARAM wParam, LPARAM lParam)
{
    switch (message)
    {
    case WM_SIZE:
